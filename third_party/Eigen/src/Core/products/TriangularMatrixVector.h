...
struct product_triangular_matrix_vector<Index,Mode,LhsScalar,ConjLhs,RhsScalar,ConjRhs,ColMajor>
{
                                     ...
                                     const RhsScalar* _rhs, Index rhsIncr, ResScalar* _res, Index resIncr, ResScalar alpha)
  {
    ...
    for (Index pis=0; pis<cols; pis+=PanelWidth)
    {
      Index actualPanelWidth = std::min(PanelWidth, cols-pis);
      for (Index k=0; k<actualPanelWidth; ++k)
      {
        Index i = pis + k;
        Index s = IsLower ? (HasUnitDiag ? i+1 : i ) : pis;
      ...
      Index r = IsLower ? cols - pis - actualPanelWidth : pis;
      if (r>0)
      {
        Index s = IsLower ? pis+actualPanelWidth : 0;
        general_matrix_vector_product<Index,LhsScalar,ColMajor,ConjLhs,RhsScalar,ConjRhs>::run(
            r, actualPanelWidth,
            &lhs.coeffRef(s,pis), lhsStride,
            &rhs.coeffRef(pis), rhsIncr,
...
struct product_triangular_matrix_vector<Index,Mode,LhsScalar,ConjLhs,RhsScalar,ConjRhs,RowMajor>
{
                  ...
                  const RhsScalar* _rhs, Index rhsIncr, ResScalar* _res, Index resIncr, ResScalar alpha)
  {
    ...
    for (Index pis=0; pis<cols; pis+=PanelWidth)
    {
      ...
      {
        Index i = pis + k;
        Index s = IsLower ? pis  : (HasUnitDiag ? i+1 : i);
      ...
      Index r = IsLower ? pis : cols - pis - actualPanelWidth;
      if (r>0)
      {
        Index s = IsLower ? 0 : pis + actualPanelWidth;
        general_matrix_vector_product<Index,LhsScalar,RowMajor,ConjLhs,RhsScalar,ConjRhs>::run(
            actualPanelWidth, r,
            &lhs.coeffRef(pis,s), lhsStride,
            &rhs.coeffRef(s), rhsIncr,
            &res.coeffRef(pis), resIncr, alpha);
      }
    }
  }
};
